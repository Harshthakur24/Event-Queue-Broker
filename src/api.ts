...
export function createApiRouter(broker: EventBroker): express.Router {
  ...
  router.post("/requests", async (req, res, next) => {
    try {
      const requestSpec: HttpRequestSpec = req.body;
      ...
      const topic = (req.query.topic as string | undefined) || "http_requests";
  ...
  router.get("/events/consume", async (req, res, next) => {
    try {
      const maxEvents = Math.min(
        Math.max(1, parseInt(req.query.maxEvents as string) || 10),
        100
      );
      const timeoutMs = Math.min(
        Math.max(0, parseInt(req.query.timeoutMs as string) || 5000),
        30000
      );
      const topic = req.query.topic as string | undefined;
  ...
  router.post("/events/acknowledge", async (req, res, next) => {
    try {
      const body: AcknowledgeRequest = req.body;
