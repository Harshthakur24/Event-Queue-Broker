...
import {
  QueueFullError,
  EventNotFoundError,
  EventNotInflightError,
  InvalidEventError,
} from "./errors";

export class EventBroker {
  private queue: AsyncQueue<EventRecord>;
  private inflight = new Map<string, InflightInfo>();
  private committed = new Set<string>();
  private receiptToEventId = new Map<string, string>(); // receiptId -> eventId
  ...
  constructor(
    ...
    private eventHandler?: (record: EventRecord) => Promise<void>
  ) {
    this.queue = new AsyncQueue<EventRecord>(config.queueMaxSize);
  }
  ...
  async enqueueEvent(payload: any, topic?: string): Promise<EnqueueResponse> {
    if (!payload || typeof payload !== "object") {
      throw new InvalidEventError("Payload must be a valid object");
    }
    ...
    if (!ok) {
      ...
      this.metrics.events.dlq++;
      throw new QueueFullError("Queue full, event moved to DLQ");
    }
  ...
  async consumeEvents(
    ...
    const events: ConsumeResponse["events"] = [];
  ...
  async acknowledgeEvents(receiptIds: string[]): Promise<AcknowledgeResponse> {
    const acknowledged: string[] = [];
    const failed: AcknowledgeResponse["failed"] = [];
  ...
  async nackEvent(receiptId: string, requeue = true): Promise<void> {
    const eventId = this.receiptToEventId.get(receiptId);
    if (!eventId) {
      throw new EventNotFoundError(`Receipt ID: ${receiptId}`);
    }

    const inflightInfo = this.inflight.get(eventId);
    if (!inflightInfo) {
      throw new EventNotInflightError(eventId);
    }
  ...
  async getDLQEntries(limit = 100, offset = 0): Promise<DLQEntry[]> {
  ...
  private startInflightReaper(): void {
    setInterval(async () => {
      const now = Date.now();
  ...
  private calculateBackoff(retries: number): number {
    const backoff = Math.min(
      this.config.baseBackoffMs * Math.pow(this.config.backoffMultiplier, retries),
